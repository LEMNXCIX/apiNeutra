...
router.post("/login", async (req, res) => {
  try {
    ...
    //?condicion
    !user && res.status(401).json("Crendenciales incorrectas:username"); //Si no se encuetra el usuario en la BD se envia esta respuesta
    ...
    passwordAuth !== req.body.password &&
      res.status(401).json("Credenciales incorrectas:password");
